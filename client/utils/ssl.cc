...
Ssl::Ssl(char *ip, int port, int userID){
	...
	if (ctx_ == NULL) fatalx("ctx");

	if (!SSL_CTX_load_verify_locations(ctx_, SSL_CA_CRT, NULL))	fatalx("verify");

	if (!SSL_CTX_use_certificate_file(ctx_, SSL_CLIENT_CRT, SSL_FILETYPE_PEM)) fatalx("cert");
	if (!SSL_CTX_use_PrivateKey_file(ctx_, SSL_CLIENT_KEY, SSL_FILETYPE_PEM)) fatalx("key");
	if (!SSL_CTX_check_private_key(ctx_)) fatalx("cert/key");
